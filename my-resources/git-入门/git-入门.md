...
5、git checkout -- bash_demo.txt # 暂存区丢弃，使工作区变干净--本地工作区文件回滚至上一次状态--暂存区的文件覆盖掉本地文件--在工作区做的修改全部撤销
git rm 文件名  # 删除（清空本地文件）再 提交
注意：清空之后，需要继续执行git commit -m ""  进行提交才能使仓库干净

回滚命令
git reset HEAD [文件名]    暂存区未commit 回滚至上次状态（将暂存区的文件移除掉，不会覆盖掉本地文件）（相当于暂存区丢弃更改(回滚)。）
git reset --hard  [版本号] 表示仓库和暂存区都回滚到指定版本（注：覆盖掉本地的文件  --hard 要小写）
...
7. 工作区是干净的，那么我们工作现场去哪里呢？我们可以使用命令 【git stash list】来查看下
    工作现场还在，Git把stash内容存在某个地方了，但是需要恢复一下，可以使用如下2个方法：
    1.git stash apply 恢复，恢复后，stash内容并不删除，你需要使用命令git stash drop来删除。
    2.另一种方式是使用【git stash pop】,恢复的同时把stash内容也删除了。
    注：git stash list # 查看隐藏的工作现场


现在我们的小伙伴要在dev分支上做开发，就必须把远程的origin的dev分支到本地？
...
推送到远程仓库分为2种。
1：本地仓库推送到远程仓库：
    首先和远程仓库建立关联：git remote add origin git@github.com...
    然后使用 git push 提交到远程仓库。
2：关联远程仓库，然后推送到远程仓库：
    克隆 git clone git@github.com...
    直接推送
